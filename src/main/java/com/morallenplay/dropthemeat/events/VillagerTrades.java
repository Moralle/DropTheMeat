    ...
    public static void onVillagersTradesEvent(VillagerTradesEvent event) {
		if(event.getType() == VillagerProfession.BUTCHER) {
			...
			if (Config.FROG_TRADE.get()) {
				event.getTrades().get(3).add(new BasicItemListing(new ItemStack(ItemInit.RAW_FROG.get(), 6), new ItemStack(Items.EMERALD, 1), 16, 4, 0.05f));
			}
			...
			if (Config.SNOW_GOLEM_TRADE.get()) {
				event.getTrades().get(5).add(new BasicItemListing(new ItemStack(ItemInit.FROZEN_GOLEM_NUGGET.get(), 4), new ItemStack(Items.EMERALD, 1), 16, 4, 0.05f));
			}
		}
	}
}
